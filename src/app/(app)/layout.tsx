...
import { TransactionSheetProvider } from "@/components/transactions/transaction-sheet-provider";
...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  // Opções do sheet de lançamento, disponível em qualquer tela.
  const { accounts, categories } = await forUser(user.id, async (tx) => ({
    accounts: await listAccounts(tx, user.id),
